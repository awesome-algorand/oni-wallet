...
export function usePrice(assetId: number) {
    ...
    const data = useMemo<number | undefined>(() => {
        ...
        return Object.keys(query.data).filter(key=>parseInt(key) === assetId).map((key) => query.data[key]["price_in_usd"])[0]
    }, [query.data]);
...
export function useIconAssets(){
  ...
  return useMemo(() => {
      if (query.isLoading || typeof query.data === 'undefined' || typeof query.data.results === 'undefined') return []

      const iconAssets = (manager.activeNetwork === "mainnet" ? iconsSnapshot : iconsTestnet).data.allAssetIcons.edges.map(({node}) => ({
          index: node.id,
          name: node.name,
