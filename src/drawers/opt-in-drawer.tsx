...
export function OptInAssetListItem({
	asset,
	info,
}: {
	info: Account;
	asset: { index: string; name: string; unitName: string; logo: string };
}) {
	const [isOptedIn, setIsOptedIn] = useState(
		info.assets
			? info.assets.some((a) => a["asset-id"].toString() === asset.index)
			: false,
	);
	...
	return (
		<ListItem
			...
			onClick={() => {
				...
				async function optInToAsset() {
					...
					const txn = makeAssetTransferTxnWithSuggestedParamsFromObject({
						...
						suggestedParams,
						assetIndex: Number.parseInt(asset.index),
					...
					const { txId } = await manager.algodClient
				...
				optInToAsset().catch((e) => {
					...
					toast({
						...
						description:
							"Ensure you have enough funds to opt-in to this asset.",
					});
				});
			}}
			...
			adornmentLeft={<img src={asset.logo} alt={`${asset.name} Logo`} />}
			adornmentRight={
				isOptedIn ? <CircleCheckBig /> : isPending ? <Spinner /> : <></>
			}
		/>
	);
}
